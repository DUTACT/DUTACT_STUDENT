...
import { useRegistrantsOfEvent } from 'src/hooks/useRegistrantsOfEvent'

interface RegistrantListPopupProps {
  eventId: number
  setIsShowPopup: React.Dispatch<React.SetStateAction<boolean>>
}

export default function RegistrantListPopup({ eventId, setIsShowPopup }: RegistrantListPopupProps) {
  const { registrants } = useRegistrantsOfEvent(eventId)
  return createPortal(
    <div
    ...
    >
      ...
      >
        <div className='flex h-header-popup items-center justify-between px-6'>
          <div className='text-base font-medium text-neutral-7'>{registrants.length} lượt đăng ký</div>
        ...
        <div className='flex max-h-main-popup min-h-[300px] flex-col gap-2 overflow-auto px-6 py-4'>
          {registrants.map((follower, idx) => (
            <>
              ...
              {idx < registrants.length - 1 && <Divider />}
            </>
          ))}
        </div>
      </div>
